...
input[['input', 'password']] = input['input'].str.split(pat=': ', expand=True)
input[['input', 'letter']] = input['input'].str.split(pat=' ', expand=True)
input[['1', '2']] = input['input']. \
    str.split(pat='-', expand=True).astype('int64')


def part_01(df:pd.DataFrame):
    df = df.rename(columns={'1': 'min_occur', '2': 'max_occur'})
    df['occur'] = df.apply(lambda x: re.findall(x['letter'], x['password']), \
        axis=1)
    df['occur'] = df['occur'].str.len()
    df['min_occur_bool'] = df.apply(lambda x: x['occur'] >= x['min_occur'], \
        axis=1)
    df['max_occur_bool'] = df.apply(lambda x: x['occur'] <= x['max_occur'], \
        axis=1)
    df['policy_bool'] = df['min_occur_bool'] & df['max_occur_bool']
    result = sum(df['policy_bool'])
    return result


def part_02(df:pd.DataFrame):
    df = df.rename(columns={'1': 'pos1', '2': 'pos2'})
    df['letter_pos1'] = df.apply(lambda x: str(x['password'])[x['pos1']-1], \
        axis=1)
    df['letter_pos2'] = df.apply(lambda x: str(x['password'])[x['pos2']-1], \
        axis=1)
    df['occur_pos1_bool'] = df.apply(lambda x: x['letter'] == x['letter_pos1'], \
        axis=1)
    df['occur_pos2_bool'] = df.apply(lambda x: x['letter'] == x['letter_pos2'], \
        axis=1)
